...
// some Qt version/configuration dependent macros to Includes or exclude certain
// code paths:
  ...
  */
  enum SizeConstraintRect {
    ...
    scrOuterRect  ///< Minimum/Maximum size constraints apply to outer rect,
                  ///< thus Includes layout element margins
  };
    ...
    while (i < lineDataSize) {
                                    ...
                                    // filter Infs which make drawPolyline block
      {
        painter->drawPolyline(
            ...
            i - segmentStart);  // i, because we don't want to Includes the
                                // current NaN point
        segmentStart = i + 1;
      }
      ++i;
